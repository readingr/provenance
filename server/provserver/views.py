...
from forms import ProfileForm, AppForm, BundleForm, SearchForm,ContactForm
from settings import ANONYMOUS_USER_ID
from models import Container
from search import search_name, search_id, search_literal, search_timeframe, search_any_text_field
...
def bundle_detail(request, container_id):
    ...
    return render_to_response('server/private/bundle_detail.html',
                              {'container': container, 'prov_n': prov_n, 
                               'prov_json': prov_json, 'license': licenses,
                               'counters': counters, 'can_delete': can_delete},
                              context_instance=RequestContext(request))
    
@permission_required_or_403('view_container', (Container, 'pk', 'container_id'))
def document_provn(request, container_id):
    container = get_object_or_404(Container, pk=container_id)
    pdbundle = container.content
    prov_g = pdbundle.get_prov_bundle() 
    prov_n = prov_g.get_provn()
    return HttpResponse(content=prov_n, mimetype='text/provenance-notation')

@permission_required_or_403('view_container', (Container, 'pk', 'container_id'))
def document_provjson(request, container_id):
    container = get_object_or_404(Container, pk=container_id)
    pdbundle = container.content
    prov_g = pdbundle.get_prov_bundle() 
    prov_json = dumps(prov_g, indent=4, cls=ProvBundle.JSONEncoder)
    return HttpResponse(content=prov_json, mimetype='application/json')
...
def create_bundle(request):
    if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                # Removing the blind submitted content
                if 'blind_submit' in request.session:
                    del request.session['blind_submit']
                return redirect(list_bundles)
            else:
                for error in form.non_field_errors():
                    messages.error(request,error)
        ...
        if 'blind_submit' in request.session:
            content = request.session['blind_submit']
            form = BundleForm(initial={'content': content})
        else:
            form = BundleForm()
        return render_to_response('server/private/create_bundle.html',
                                  {'form': form},
...
@csrf_exempt
def submit_blind(request):
    if 'blind_submit' in request.session:
        del request.session['blind_submit']
    request.session['blind_submit'] = request.POST['content']
    print request.session['blind_submit']
